...
- **公開APIのレート制限 — 実装・デプロイ済み(2026-09-16)**: 下の「納品までに対応する」1.で洗い出した`GET /reservations/lookup`(電話番号+連番のreservation_numberでの総当たり)対策として、固定ウィンドウ方式のレート制限を実装した。
  ...
  - **Cloudflare Turnstile — 実装・デプロイ・実機確認済み(2026-09-16)**: `POST /reservations`にボット対策を追加。
    - バックエンド: `_shared/turnstile.ts`(`verifyTurnstile()`)を新設。`TURNSTILE_SECRET_KEY`未設定時・Cloudflare APIとの通信失敗時はfail-open(検証をスキップして通す。RESEND_API_KEY等と同じ「未設定/障害で予約自体を止めない」方針)。`reservations/create.ts`で`body.turnstile_token`を検証し、失敗時は`VALIDATION_ERROR`で弾く
    - フロントエンド: `lp/reserve.html`にTurnstileウィジェット(`<div class="cf-turnstile">`)を追加し、`lp/js/reserve.js`で`render=explicit`モードにより`window.CITY_DOGS_CONFIG.TURNSTILE_SITE_KEY`を使って明示的に描画。取得したトークンを`POST /reservations`のボディに含める。送信後・失敗後は`turnstile.reset()`でウィジェットをリセット(トークンは1回使うと無効になるため)
    - サイトキー・シークレットキーは、Cloudflare公式のテスト専用キー(`1x00000000000000000000AA`/`1x0000000000000000000000000000000AA`、常に成功)を暫定設定。ユーザーのCloudflareアカウント開設を待たずに実装・検証まで完了できた。本番運用前に実際のサイトキーに差し替えが必要(`lp/js/config.js`の🏪マーカー参照)
    - **⚠️ 実際に踏んだ教訓**: Turnstileの`<script src="...api.js?onload=onTurnstileLoad...">`に`async defer`と両方付けていたところ、HTML仕様上`async`が優先されて`defer`が無視され、`window.onTurnstileLoad`(body末尾の`reserve.js`で定義)がまだ定義されていないタイミングでこのスクリプトが実行されてしまい、コールバックが見つからない警告が出た(実機で発生)。`async`を外し`defer`のみにして解決。**教訓: `async`と`defer`を両方指定しても`defer`にはならない(asyncが勝つ)。実行順序を保証したい外部スクリプトには`defer`のみを使うこと**
    - **⚠️ 別の副作用として発覚**: `ALLOWED_ORIGINS`を本番の2オリジンに絞ったこと(下記5.)で、ローカルE2Eテスト(`http://localhost:5500`等)がCORSでブロックされるようになっていた。`lp/tests/*.e2e.mjs`・`booking/admin/tests/admin.e2e.mjs`が使う固定ポート(5500/5501/5502)を`ALLOWED_ORIGINS`に追加して解決。**教訓: CORSを本番オリジンだけに絞る際、ローカルE2Eテストが使う固定ポートも同時に許可リストへ入れないと、既存のテストスイート全体が壊れる**
    - 実機確認: `lp/tests/reserve.e2e.mjs`をTurnstile組み込み後に実行し、実際の予約作成(B000000039、B000000040)・コンソールエラーなしを確認済み。同テストにTurnstileトークン生成を待つ処理(固定waitではなくポーリング)を追加済み
    - **本番用サイトキー・シークレットキーを発行・設定済み(2026-09-16)**: `lp/js/config.js`の`TURNSTILE_SITE_KEY`とSupabaseの`TURNSTILE_SECRET_KEY`をCloudflareダッシュボードで発行した実キーに差し替え。Hostname Managementには`city-dogs.khs-w098style.workers.dev`(本番)と`localhost`(ローカルE2Eテスト用)の両方を登録済み
    - **⚠️ 実際に踏んだ教訓(本番キーとテストキーは組み合わせを崩せない)**: 本番用サイトキーに切り替えた直後、`reserve.e2e.mjs`がTurnstileのトークン取得待ちでタイムアウトするようになった。原因は、本番用サイトキーは**実際にボット検知を行う**ため、Playwrightのヘッドレスブラウザが正当にボットとして検知され、永久にトークンが発行されなかったこと(Cloudflareのテスト専用サイトキーだけが無条件で成功する特別仕様だった)。フロントエンドだけテスト専用サイトキーに差し替えて回避しようとしたところ、今度はバックエンドが「本番のシークレットキーはテスト用トークンを拒否する」というCloudflareの仕様に阻まれ、`VALIDATION_ERROR`(ボット判定)になった。**結論: テスト用サイトキーとテスト用シークレットキーは必ずペアで使う必要があり、フロントエンド・バックエンドの片方だけ差し替えても機能しない**。`reserve.e2e.mjs`は`page.route()`でconfig.js配信時だけテスト用サイトキーに差し替える仕組みにしたが、バックエンドの`TURNSTILE_SECRET_KEY`は自動化できないため、**このE2Eテストを実行する前後で手動により一時的にテスト用シークレットキーへ切り替え→実行→本番キーに戻す、という手順が必要**(`reserve.e2e.mjs`冒頭のコメント参照)。戻し忘れると本番のボット対策が無効化されたままになるため要注意
- **⚠️ 納品までに対応する(2026-09-16、ユーザー確定)— セキュリティ関連の残課題**: RLS対応(上記)は完了したが、そのレビュー中に洗い出した以下は納品前に対応する方針で合意済み
  1. **公開APIのレート制限・Cloudflare Turnstile**: `POST /reservations`のボット対策に加え、`GET /reservations/lookup`は`reservation_number`が`B000000001`のような連番のため、電話番号(攻撃者が知っている前提)と組み合わせた総当たりで他人の予約情報を引き当てられるリスクがある。**→レート制限・Turnstileともに実装済み(上記)。残るは本番用サイトキーへの差し替えのみ**
...
- **ホスティング先をCloudflare Workers(静的アセット配信)に決定・LP/管理画面ともデプロイ済み(2026-09-16)**: GitHubリポジトリ(`city-dogs`)をCloudflare Workers & Pagesと連携し、2つのプロジェクトを作成済み。
  - LP: `https://city-dogs.khs-w098style.workers.dev`(Path設定を`lp`にして作成)
  - 管理画面: `https://city-dogs-admin-deploy.khs-w098style.workers.dev`(Path設定を`booking/admin`にして作成)
  - **🚧 `ALLOWED_ORIGINS`・`MANAGE_PAGE_BASE_URL`のsecretを、上記の暫定`.workers.dev`URLで設定済み(2026-09-16)**。`ALLOWED_ORIGINS`はLP・管理画面両方のオリジンをカンマ区切りで指定(CORS実機テストで、指定した2オリジンのみ許可・それ以外は拒否されることを確認済み)。**独自ドメインが決まったら、この2つのsecretを新しいドメインの値で必ず再設定すること**(ユーザーの明示的な方針: 「独自ドメインが決まったら再度更新」)。`RESEND_FROM_ADDRESS`は`.workers.dev`のサブドメインではResend側のドメイン所有権検証ができないため、独自ドメイン確定まで引き続き保留
  - **その後`ALLOWED_ORIGINS`にローカルE2Eテスト用の固定ポート(`http://localhost:5500`/`5501`/`5502`)も追加(2026-09-16)**。本番オリジンだけに絞った直後、`lp/tests/*.e2e.mjs`・`booking/admin/tests/admin.e2e.mjs`がCORSでブロックされて動かなくなることが判明したため。独自ドメイン更新時もこの3つのlocalhostエントリは残すこと
  - **⚠️ 重要な確認結果**: 長らく懸念していた「`manage.html?token=...`のようなクエリ文字列付きURLが、ホスティング先のクリーンURL機能で欠落しないか」を実機で検証した。Cloudflare Workersは`/manage.html`を`/manage`に307リダイレクトする(拡張子なしの「クリーンURL」)が、**クエリ文字列(`?token=...`)はリダイレクト時にきちんと保持され、最終的に200 OKで正しいページが表示される**ことを確認した。ローカルの`serve`パッケージで踏んだ「クエリ文字列が消える」不具合はCloudflare Workersでは発生しない
  - `lp/.assetsignore`・`booking/admin/.assetsignore`(`node_modules`・`tests/`・`package.json`等を配信対象から除外)は、ローカルの`wrangler deploy --dry-run`では効果が確認できなかった(ファイル数が変化しなかった)が、**実際のCloudflareへのデプロイでは正しく機能しており**、`/package.json`・`/tests/reserve.e2e.mjs`等が404になることを実機で確認した。**教訓: `wrangler deploy --dry-run`はCloudflare APIとの通信(アセットの実際のフィルタリング処理を含む)をスキップするため、`.assetsignore`の効果を検証する手段としては信頼できない。実際に確かめるには本物のデプロイが必要**
  - `lp/wrangler.jsonc`・`booking/admin/wrangler.jsonc`(Workers静的アセット配信の設定、`assets.directory: "."`)を新設。CloudflareのGit連携フローが「Workers」として扱い、Deploy commandに`npx wrangler deploy`を要求してくるため、この設定ファイルなしではデプロイが失敗する
  - 環境変数(Cloudflare側の"Variable name/value")・APIトークンはWorkers側のバインディングとして不要(SupabaseのURL/anonキーは`lp/js/config.js`にビルド前から埋め込み済みのため)。デプロイ用のAPIトークンはCloudflareが自動生成するもので問題ない
  - 次の手順: 独自ドメイン取得・アタッチ→`ALLOWED_ORIGINS`/`MANAGE_PAGE_BASE_URL`/`RESEND_FROM_ADDRESS`のsecretを新ドメインの値に更新(TEMPLATE.mdの該当項目を参照)
